...
\subsubsection{Server Manager - Dogyeom Kim}
\begin{adjustwidth}{1em}{0em}
\setlength{\parindent}{1em}
The Server Manager is responsible for ensuring stable backend deployment, scalable infrastructure, and reliable system operation.
They manage the AWS EC2 instance hosting the FastAPI backend, configure NGINX as a reverse proxy, and maintain Docker-based container orchestration.

Their responsibilities include setting up secure connections, monitoring system performance, and configuring logs using system tools such as CloudWatch or Docker logs.
They also implement CI/CD pipelines using GitHub Actions, automate Docker image builds, and maintain environment variables and version management across development and production environments.

They ensure the backend remains stable under real-time streaming from iOS/watchOS devices, manage API rate limits for OpenAI and KMA, and monitor failures or latency issues across LLM-based automation tasks.
Their work ensures that the HARU system can reliably process biometric streams, execute real-time predictions, and deliver seamless home-automation responses.
\end{adjustwidth}