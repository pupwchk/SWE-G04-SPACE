...
# Docker 환경에서는 api:11325, 로컬에서는 localhost:8000
BASE_URL = os.getenv("TEST_BASE_URL", "http://localhost:8000")
TEST_USER_ID = str(uuid.uuid4())  # UUID 형식으로 변경
...
def test_geofence_config():
    ...
    # user_id를 파라미터로 전달
    response = requests.get(f"{BASE_URL}/api/location/geofence/config/{TEST_USER_ID}")
    print(f"Status: {response.status_code}")
    if response.status_code == 200:
        print(f"Response: {json.dumps(response.json(), indent=2, ensure_ascii=False)}")
    else:
...
def test_webhook_chat():
    """채팅 웹훅 시뮬레이션 테스트"""
    print_section("5. 채팅 웹훅 시뮬레이션 테스트")

    webhook_payload = {
        "category": "group_channel:message_send",
        "app_id": "89419626-76B6-4551-AB9B-D7FF2B41A68D",
        "channel": {
            "channel_url": f"test_channel_{TEST_USER_ID}",
            "name": "Test Channel"
        },
        "sender": {
            "user_id": TEST_USER_ID,
...
def test_sendbird_user():
    ...
    print("   Dashboard에서 home_ai_assistant 사용자를 미리 생성해야 합니다.")
...
def main():
    ...
    try:
        ...
        print("   로컬: uvicorn app.main:app --reload --host 0.0.0.0 --port 8000")
